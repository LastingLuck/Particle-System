...
int main(int argc, char** argv) {
    ...
    objmodel = glm::rotate(objmodel, (float)M_PI/6.0f, glm::vec3(1.0f, 0.0f, 0.0f));
    //objmodel = glm::scale(objmodel, glm::vec3(7.0f, 1.0f, 7.0f));
    ...
    Particle p;
    Emitter* emitter = new Emitter(400);
    emitter->setColor(glm::vec3(0.4f, 0.4f, 0.4f));
    emitter->setPosition(glm::vec3(0.0f, -0.48f, 0.0f));
    emitter->setVelocity(0.5f);
    emitter->setDirection(glm::vec3(0.0f, 1.0f, 0.0f));
    emitter->setParticleLife(6.0f);
    emitter->setDirectionRand(0.03f);
    emitter->setLifeRand(0.05f);
    emitter->setColorRand(0.1f);
    emitter->setShaderProgram(shaderProgram);
    emitter->setRadius(0.2f);
    emitter->setBounciness(0.0f);
    emitter->setTermVelocity(1.0f);
    emitter->setGravity(glm::vec3(0.0f, 9.8f, 0.0f));
    
    //SDL_SetWindowTitle(window, "FPS: 0    0 Particles");
    int numFrames = 0;
    ...
    while(true) {
        while(SDL_PollEvent(&windowEvent)) {
            ...
            else if(windowEvent.type == SDL_KEYUP && windowEvent.key.keysym.sym == SDLK_t) {
                emitter->toggleTexture();
                if(emitter->usingTexture()) {
                    printf("Texture On\n");
                }
                else {
                    printf("Texture Off\n");
        ...
        if(testIntersect) {
            if(wind) {
                emitter->update(triverts, vSize, modelPos, numVerts1, glm::vec3(0.0f, 0.0f, 0.5f));
            }
            else {
            ...
            if(wind) {
                emitter->update(0, 0, modelPos, numVerts1, glm::vec3(0.0f, 0.0f, 0.5f));
            }
            else {
