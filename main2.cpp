...
float camx = -5.0f, camy = 3.5f, camz = 0.0f;
float dirx = 1.0f/sqrt(2), diry = -1.0f/sqrt(2), dirz = 0.0f;
...
int main(int argc, char** argv) {
    ...
    objmodel = glm::scale(objmodel, glm::vec3(3.0f, 1.0f, 3.0f));
    modelPos.push_back(objmodel);
    objmodel = glm::mat4();
    objmodel = glm::translate(objmodel, glm::vec3(0.0f, 0.5f, 1.2f));
    objmodel = glm::rotate(objmodel, 0.78539816339f, glm::vec3(1.f, 0.f, 0.f)); //45 deg
    modelPos.push_back(objmodel);
    objmodel = glm::mat4();
    objmodel = glm::translate(objmodel, glm::vec3(0.0f, 0.5f, -1.2f));
    objmodel = glm::rotate(objmodel, -0.78539816339f, glm::vec3(1.f, 0.f, 0.f)); //30 deg
    modelPos.push_back(objmodel);
    objmodel = glm::mat4();
    objmodel = glm::translate(objmodel, glm::vec3(1.2f, 0.5f, 0.0f));
    objmodel = glm::rotate(objmodel, 0.78539816339f, glm::vec3(0.f, 0.f, 1.f)); //30 deg
    modelPos.push_back(objmodel);
    objmodel = glm::mat4();
    objmodel = glm::translate(objmodel, glm::vec3(-1.2f, 0.5f, 0.0f));
    objmodel = glm::rotate(objmodel, -0.78539816339f, glm::vec3(0.f, 0.f, 1.f)); //30 deg
    ...
    Particle p;
    for(int i = 0; i < 400; i++) {
        glm::vec3 randpos = glm::vec3((float)rand()/RAND_MAX*2-1, 3.0f, (float)rand()/RAND_MAX*2-1);
        p = Particle(randpos, glm::vec3(0), glm::vec3(1.0f, 0.0f, 0.0f));
        plist.push_back(p);
    }
    ...
    while(true) {
        ...
        for(int i = 0; i < (signed)plist.size(); i++) {
            ...
            model = glm::mat4();
            model = glm::translate(model, pp->getPosition());
            ...
            if(run) {
               if(testIntersect) {
                    if(wind) {
                        pp->update(triverts, vSize, modelPos, numVerts1, usegrav, glm::vec3(1.0f, 0.0f, 0.0f));
                    }
                    else {
                        pp->update(triverts, vSize, modelPos, numVerts1, usegrav);
                    }
                } 
            }
        }
